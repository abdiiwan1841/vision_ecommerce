      ...
      <table class="table table-bordered table-striped table-hover mt-3">
        ...
        @foreach ($sizes as $key => $size)
            <tr>
                ...
                <td>{{$size['name']}}</td>
            <td>
            <button class="btn btn-primary btn-sm" id="open_modal" onclick="EditProcess({{$size['id']}},'{{route('sizes.update',$size['id'])}}')"  data-baseurl="{{asset('')}}" ><i class="fas fa-edit"></i></button> 
         
            
              <form id="delete-from-{{$size['id']}}" style="display: inline-block" action="{{route('sizes.destroy',$size['id'])}}" method="POST">
                    @csrf
                    @method('DELETE')
