...
@section('content')
  ...
  <div class="row">
    <div class="col-lg-12">
        ...
        <div class="p_wrapper">
          <div class="row">
            <div class="col-lg-4"><a href="{{route('returnproduct.index')}}" class="btn btn-sm btn-dark"><i class="fa fa-angle-left"></i> back</a></div>
            <div class="col-lg-8">
            <strong class="float-right">EDIT SALES RETURN INVOICE # {{$return->id}}</strong> 
            </div>
          </div>
          <hr>
           <div class="row">
            ...
            <div class="col-lg-4">
              <div class="form-group">
                <label for="return_date">Date <button onclick="changeDate()" class="btn btn-link">(change)</button></label>
                @php
    ...
    <div class="col-lg-12">
      ...
      <h3 class="text-center">RETURN INVOICE # {{$return->id}}</h3>
        <h5 class="date"></h5> <br>
        <div class="row">
            <div class="col-lg-6">
    ...
    <div class="table-responsive">
      ...
      <div class="col-lg-5" id="amount-info">
        ...
        <table class="table table-bordered">
          ...
          <tr>
            ...
            <td>Discount Amount <input type="text" class="discount form-control" value="{{$return->discount}}"></td>
          </tr>
          <tr>
            <td>Net Amount</td>
          ...
          <tr>
          <td>Carrying & Loading <input type="number" class="form-control tiny-input" id="carrying_and_loading" value="{{$return->carrying_and_loading}}"></td>
      ...
      <button type="button" class="btn btn-dark"  id="confirm-btn" onclick="confirm_return()">Confirm</button>
...
$.get("{{route('returninfo.api',$return->id)}}",function(data, status){
  if(status === 'success'){
    if(sessionStorage.returnId == undefined ){
    sessionStorage.setItem('posreturnCart',JSON.stringify(data.returnCart));
    sessionStorage.setItem('user_id', data.user_id);
    sessionStorage.setItem('return_date' ,data.return_date);
    sessionStorage.setItem('returnId' ,data.return_id );
    location.reload();
  }else if(sessionStorage.returnId != data.return_id){
...
var baseuel = '{{url('/')}}';
  ...
  if(sessionStorage.return_date != undefined){
     $('.date').html('Sales Date: '+sessionStorage.return_date);
  }
...
$("#discount_input").on("input",function(){
    ...
    sales_discount =  net_discount/100;
      var disc_amout = posreturnCart.totalCart()*sales_discount;
...
function displayCart() {
  ...
  var disc = subtotal*order_discount;
 ...
 var discount_amount = $(".discount").val();
   if(sessionStorage.posreturnCart.length < 3){
    ...
    $.ajax({
          ...
          url: "{{route('returnproduct.update',$return->id)}}",
          type: "PUT",
          dataType: 'json',
          success: function (data) {
            console.log(data);
              sessionStorage.clear();
              var b_url = '{{url('/')}}';
              window.location = b_url+'/admin/pos/returnproduct/'+data
