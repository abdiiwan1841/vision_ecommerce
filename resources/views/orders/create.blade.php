...
@section('content')
  ...
  <div class="row">
    <div class="col-lg-4">
        ...
        <div class="p_wrapper">
          <div class="row">
            <div class="col-lg-6"><a href="{{route('order.index')}}" class="btn btn-sm btn-success"><i class="fa fa-angle-left"></i> back</a></div>
            <div class="col-lg-6">
               <strong class="float-right">CREATE ORDERS</strong> 
            </div>
          </div>
           <br><br>
          <div class="card">
          ...
          <form id="left-form">
            <div class="form-group">
              ...
              @endphp
              <input type="text" class="form-control" name="order_date" id="order_date" value="{{$mytime->toDateString()}}" readonly>
    ...
    <div class="col-lg-8">
      ...
      <div class="col-lg-7" id="amount-info">
        ...
        <table class="table table-bordered">
          ...
          <tr>
          <td>Shipping <input type="number" class="form-control tiny-input" id="shipping" value="{{$charge->shipping}}"></td>
            <td class="shipping">0</td>
          </tr>
          <tr>
            <td>VAT & TAX ({{$charge->vat+$charge->tax}}) %</td>
            <td class="vat">0</td>
          </tr>
      ...
      <button type="button" class="btn btn-success text-center"  id="confirm-btn" onclick="confirm_order()">Confirm</button>
  ...
  obj.addItemToCart = function(name, price, count, id,o_name,image) {
    for(var item in cart) {
      if(cart[item].name === name) {
        alert('Oops... This Product Is Already Added');
        return;
      }
    }
...
var VAT_PERCENTAGE = '{{$charge->vat+$charge->tax}}';
...
function displayCart() {
  ...
  var netamount = Math.round(subtotal-disc);
  var vat = strip(parseFloat(netamount)*(VAT_PERCENTAGE/100));
  var g_total = Math.round(parseFloat(netamount)+parseFloat(shipping)+parseFloat(vat));
