    ...
    <div class="card-header">
      <div class="row">
        ...
        <div class="col-lg-6 text-right">
          <a  href="{{route('employee.create')}}" class="btn btn-info btn-sm">+ Add New Employee</a>
        </div>
        ...
        <tbody>
         @foreach ($employees as $key => $item)
          <tr>
            ...
            <td>{{$item->name}}</td>
            <td>{{$item->phone}}</td>
            <td>{{$item->employee_type->name}}</td>
          <td><a class="btn btn-sm btn-primary" href="{{route('employee.edit',$item->id)}}"><i class="fas fa-edit"></i></a> | <a class="btn btn-sm btn-info" href="{{route('employee.show',$item->id)}}"><i class="fas fa-eye"></i></a>
          {{-- <form style="display: inline-block" action="{{route('employee.destroy',$item->id)}}" method="POST">
              @csrf
              @method('DELETE')
